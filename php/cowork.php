...
include('../php/db.php');
	...
	if ($type == 'cowork') {
		...
		$query = sprintf("INSERT INTO `1_COWORK_TASK` (TASKID,COWORKER,COWORK) VALUES ('%s','%s','%s')",mysql_real_escape_string($taskid), mysql_real_escape_string($wantcoworker), mysql_real_escape_string('1'));
		$result = mysql_query($query);
		if( !$result ){
		...
		$query = mysql_query("UPDATE `1_INFO3_TASK` SET NUM_COWORK = '$COWORK' WHERE TASKID = '$taskid'");
		if( !$query ){
	...
	if ($type == 'refuse') {
		...
		$query = sprintf("INSERT INTO `1_COWORK_TASK` (TASKID,COWORKER,COWORK) VALUES ('%s','%s','%s')",mysql_real_escape_string($taskid), mysql_real_escape_string($wantcoworker), mysql_real_escape_string('0'));
		$result = mysql_query($query);
		if( !$result ){
