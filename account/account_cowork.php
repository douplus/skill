...
$userid = $a -> userid;
	...
	$query = sprintf("SELECT 1_CV.USERNAME FROM `1_CV`,`1_TASK`,`1_CHECK_TASK` WHERE 1_CV.USERID = 1_CHECK_TASK.COWORKER AND 1_CHECK_TASK.TASKID =  1_TASK.TASKID ORDER BY 1_CHECK_TASK.TIMESTAMP ");
	$result = mysql_query($query) or die('error@錯誤。');
		while( $a = mysql_fetch_array($result) ){
        $want_cowork[] = $a['USERNAME'];
        }
	$query = sprintf( "SELECT 1_CHECK_TASK.TIMESTAMP,1_CHECK_TASK.COWORKER,1_CV.USERID,1_CV.USERNAME,1_CV.USER_PHOTO,1_TASK.TITTLE,1_TASK.TASKID,1_TASK.TASKPOSTERID FROM `1_CHECK_TASK`,`1_CV`,`1_TASK` WHERE 1_CV.USERID = 1_TASK.TASKPOSTERID AND 1_CHECK_TASK.TASKID =  1_TASK.TASKID ORDER BY 1_CHECK_TASK.TIMESTAMP" );
	$result = mysql_query($query) or die('error@錯誤。');
		...
		$num=count($check_task);$checkhtml='';
			...
			$checkhtml.='<li>'.	
							'<div class="thumbnail">'.
								'<a href="../profile/index.php?u='.$a[7].'&amp;v='.$userid.'" title="'.$a[3].'"><img width="50" height="50" src="../photo/'.$a[7].'.jpg'.'" title="'.$a[3].'"></a>';
				$checkhtml.='</div>'.
				...
				$coworkhtml.='<li>'.	
				  ...
				  $coworkhtml.='</div>'.
				   ...
				   $coworkhtml.='<span class="num chinese">'.$a[9].' views │ '.$a[10].' answers │ '.$a[11].' coworks</span>'.
								'<br>'.
								'<br>'.
								'<a href="./cooperation/index.php?cooperation_id='.$a[5].'">前往合作討論區</a>'.
...
$userid = $a -> userid;
	//dynamic add check check
	$query = sprintf("SELECT 1_CV.USERNAME FROM `1_CV`,`1_TASK`,`1_CHECK_TASK` WHERE 1_CV.USERID = 1_TASK.TASKPOSTERID ORDER BY 1_CHECK_TASK.TIMESTAMP ");
	$result = mysql_query($query) or die('error@錯誤。');
		while( $a = mysql_fetch_array($result) ){
        $taskposter[] = $a['USERNAME'];
        }
	$query = sprintf( "SELECT 1_CHECK_TASK.TIMESTAMP,1_CHECK_TASK.COWORKER,1_CV.USERID,1_CV.USERNAME,1_CV.USER_PHOTO,1_TASK.TITTLE,1_TASK.TASKID,1_TASK.TASKPOSTERID FROM `1_CHECK_TASK`,`1_CV`,`1_TASK` WHERE 1_CV.USERID = 1_CHECK_TASK.COWORKER AND 1_CHECK_TASK.TASKID =  1_TASK.TASKID ORDER BY 1_CHECK_TASK.TIMESTAMP" );
	$result = mysql_query($query) or die('error@錯誤。');
		//0:USERID 1:USERNAME 2:USER_PHOTO 3:TITTLE 4:TIMESTAMP 5:TASKID 6:COWORKER 7:TASKPOSTERID
		while( $a = mysql_fetch_array($result) ){
        $check_task[] = $a['USERID'].'***'.$a['USERNAME'].'***'.$a['USER_PHOTO'].'***'.$a['TITTLE'].'***'.$a['TIMESTAMP'].'***'.$a['TASKID'].'***'.$a['COWORKER'].'***'.$a['TASKPOSTERID'];
		...
		$num=count($check_task);$checkhtml='';
			...
			$checkhtml.='<li>'.	
							'<div class="thumbnail">'.
								'<a href="../profile/index.php?u='.$a[7].'&amp;v='.$userid.'" title="'.$a[3].'"><img width="50" height="50" src="../photo/'.$a[7].'.jpg'.'" title="'.$a[3].'"></a>';
				$checkhtml.='</div>'.
							'<h3 class="chinese"><a href="../profile/index.php?u='.$a[7].'&amp;v='.$userid.'">'.$taskposter[$i].'</a> 想要和你合作 ：「 <a href="../task/discuss/index.php?task_id='.$a[5].'">'.$a[3].'</a> 」。</h3>'.
							'<button class="account_co_btn ensure_btn" type="button"  taskid="'.$a[5].'" userid="'.$a[0].'" >確認</button>'.
							'<button class="account_co_btn refuse_btn" type="button"  taskid="'.$a[5].'" userid="'.$a[0].'" >拒絕</button>'.						
							'<br>';
				$checkhtml.='<span class="date chinese">'.$a[4].'</span>'.
